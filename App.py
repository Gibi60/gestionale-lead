...
with tab_scheda:
    elenco_aziende = df_lead['Ragione Sociale'].unique().tolist()
    ...
    with tab_note:
        stato_wf = st.selectbox("Stato Workflow:", ["Importato", "In Analisi", "Audit Generato", "Contattato", "In Trattativa", "Vinto", "Perso"], 
                                index=["Importato", "In Analisi", "Audit Generato", "Contattato", "In Trattativa", "Vinto", "Perso"].index(str(lead_info.get('Stato Workflow', 'Importato'))))
    ...
    with tab_validazione:
        ...
        if st.button("💾 Salva Validazione"):
            df_lead.at[idx_row, 'Score Opportunità (%)'] = 100 # Esempio calcolo
            save_data(df_lead)
